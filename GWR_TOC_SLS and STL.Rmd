...
STationer byter mvm-id och SO4 byter till SO4_IC. behöver bearbetas innan analysen kan göras.
...
#bw<-bw.gwr(cent_toc ~ cyear,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes_all.spdf)
```

Model for the combined dataset.
```{r}
lakes_all.spdf <- SpatialPointsDataFrame(lakes_all[, c(3,4)], lakes_all)
model_toc_SLS_and_STL<-gwr.basic(cent_toc ~ cyear,bw=400,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes_all.spdf)
...
```{r}
ggplot(lakes_all, aes(x=stationskoordinat_e_y,y=stationskoordinat_n_x))+
  ...
  geom_point(aes(colour=model_toc_SLS_and_STL$SDF$cyear))+
...
```{r}
lakes_all%>%mutate(pval0=case_when(pval_toc_SLS_and_STL$SDF$cyear_p<0.05 & model_toc_SLS_and_STL$SDF$cyear>0~1,
                              pval_toc_SLS_and_STL$SDF$cyear_p<0.05 & model_toc_SLS_and_STL$SDF$cyear<0 ~-1,
                              TRUE~0))%>%
...
model_toc_SLS<-gwr.basic(cent_toc ~ cyear,bw=382,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes_SLS.spdf)
...
```{r}
lakes_all%>%mutate(coef_all=model_toc_SLS_and_STL$SDF$cyear,
                pval_all=pval_toc_SLS_and_STL$SDF$cyear_p)->lakes_all1

lakes_SLS%>%mutate(coef_SLS=model_toc_SLS$SDF$cyear,
                  pval_SLS=pval_toc_SLS$SDF$cyear_p)%>%right_join(lakes_all1, by=c("ID","year"))%>%
...
ggplot(diffs, aes(x=stationskoordinat_e_y.x,y=stationskoordinat_n_x.x))+
  geom_polygon(data = border1,aes(x = East_SWERE, y = North_SWER),fill = "gray",alpha = 0.25)+
  # geom_polygon(data = border_gotland,aes(x = East_SWERE, y = North_SWER),fill = "gray",alpha = 0.25)+
    geom_polygon(data = border_vänern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
    geom_polygon(data = border_vättern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
    geom_polygon(data = border_mälaren,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_point(aes(colour=est_diff))+
    geom_path(data = border1,aes(x = East_SWERE, y = North_SWER))+
 # geom_path(data = border_gotland,aes(x = East_SWERE, y = North_SWER))+
   geom_path(data = border_vänern,aes(x = East_SWERE, y = North_SWER))+
   geom_path(data = border_vättern,aes(x = East_SWERE, y = North_SWER))+
   geom_path(data = border_mälaren,aes(x = East_SWERE, y = North_SWER))+
   coord_fixed()+
...
Figure S2: Differences in significance for models based on SLS and STL compared to SLS only
```{r}
...
Comparision of trends in SLS and STL for specific identified areas
...
lakes_all%>%mutate(dist=(stationskoordinat_e_y-450408)^2 +(stationskoordinat_n_x-6331514)^2)%>%filter(dist <= distance$dist & sls==1)%>%
...
lakes_all%>%mutate(dist=(stationskoordinat_e_y-384420)^2 +(stationskoordinat_n_x-6284776)^2)%>%filter(dist <= distance$dist & sls==1)%>%
  mutate(ID_new=paste(ID, sls))%>%
  ggplot()+
  geom_point(aes(x=year, y=log_toc), col="gray40")+
  geom_line(aes(x=year, y=log_toc, group=ID), col="gray40")+
...
lakes_all%>%mutate(dist=(stationskoordinat_e_y-647006)^2 +(stationskoordinat_n_x-6538408)^2)%>%filter(dist <= distance$dist & sls==1)%>%
  mutate(ID_new=paste(ID, sls))%>%
  ggplot()+
  geom_point(aes(x=year, y=log_toc), col="gray40")+
  geom_line(aes(x=year, y=log_toc, group=ID), col="gray40")+
...
lakes_all%>%mutate(dist=(stationskoordinat_e_y-496102)^2 +(stationskoordinat_n_x-6225833)^2)%>%filter(dist <= distance$dist & sls==1)%>%
  mutate(ID_new=paste(ID, sls))%>%
  ggplot()+
  geom_point(aes(x=year, y=log_toc), col="gray40")+
  geom_line(aes(x=year, y=log_toc, group=ID), col="gray40")+
...
Nonlinear trends for the STL Program usign generalised additive models
...
trendlakes2012<- import_slu_mvm_data_excel("STL2012-2021.xlsx", numeric_var = 26)%>%
  clean_names()%>%
  #filter(provtagningsmanad>8 & provtagningsmanad <12 & provtagningsar>=2007)%>%
...
results1 %>%plot_screeningtrends_pvalues(y_id=overvakningsstation, sorting=desc(stationskoordinat_n_x))
