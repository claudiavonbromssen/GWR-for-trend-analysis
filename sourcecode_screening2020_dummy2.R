...
modeling <- function(x, link = "identity", autocor = autocor, tdist = tdist) {
  ...
  formula <- variable ~ s(decimaldate, k = round(nrow(x) / 2)) + seas
...
screeningmodeling <- function(.data,
  ...
  .data %>%
    mutate(variable = !!variable,
           date = !!datevar,
           seas=!!seas) %>%
    select(date, variable, seas, !!!nestvars) %>%
    group_by(!!!nestvars, date, seas) %>%
...
plot_screeningtrends_pvalues <- function(.output, y_id = NULL, sorting = NULL, wrappingvar = NULL){
  ...
  plotobj <- data %>%
    ...
    scale_fill_gradient2(low = "#56B4E9", mid="#FFFFFF",
...
plot_screeningtrends_relative <- function(.output, y_id = NULL, sorting = NULL, wrappingvar = NULL){
  ...
  plotobj <- data %>%
    ...
    ggplot(aes(y=!!y_id,
               ...
               xmax = end))+
    geom_rect()+
    scale_fill_gradient2(low = "#56B4E9", mid="#FFFFFF",high = "#D55E00", midpoint = 0, # breaks=seq(-2,2,0.5),
...
plot_individual_trend <- function(x, y=NULL, title=NULL){
  ...
  annualterm <- predict(x$fit[[1]], newdata=x$data[[1]], type="terms")[,"s(decimaldate)"]
