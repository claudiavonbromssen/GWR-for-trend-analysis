...
```{r}
lakes2%>%group_by(ID, stationskoordinat_e_y, stationskoordinat_n_x)%>%summarize(var_log_toc=var(log_toc),sd_log_toc=sd(log_toc), mean_log_toc=mean(log_toc),var_toc=var(toc), mean_toc=mean(toc), cv_toc=var_toc/mean_toc, cv_log=sqrt(exp((sd_log_toc*log(10))^2)-1))->var_data
```

Figure 2, left: Plot of mean TOC in Sweden.
```{r}
var_data%>%ggplot( aes(x=stationskoordinat_e_y,y=stationskoordinat_n_x))+
...
```{r}
var_data%>%arrange(cv_log)%>%ggplot( aes(x=stationskoordinat_e_y,y=stationskoordinat_n_x))+
...
Identify lakes with high coefficient of variation. Save information for table S1.
```{r}
var_data%>%filter(cv_log>0.8)%>%mutate(ind=1)->high_cv
...
The station-wise mean-centered TOC variable is computed. Intercept should be the overall mean (i.e. close to zero), i.e. the variable year is centered. 
```{r}
...
lakes2%>%
  left_join(lakes_sd)%>%mutate(cent_toc=log_toc-mean_toc,
         cyear=year-2014)->lakes3
```
...
Bandwidth search for mean-centered data. This is only run once with a result of 382. Takes time to run.
```{r}
# 
# lakes.spdf <- SpatialPointsDataFrame(lakes3[, c(5,4)], lakes3)
# bw<-bw.gwr(cent_toc ~ cyear,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes.spdf)
```




Main model for TOC, computed on mean-centered TOC with the cross-validated knn (382).
```{r}
lakes.spdf <- SpatialPointsDataFrame(lakes3[, c(5,4)], lakes3)
model_toc_382<-gwr.basic(cent_toc ~ cyear,bw=382,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes.spdf)

pval_toc_382<-gwr.t.adjust(model_toc_382)
```

Model for non-centered TOC for comparison, cross-validated knn is 23.
```{r}
#bw<-bw.gwr(log_toc ~ cyear,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes.spdf)
model_toc_23_noncent<-gwr.basic(log_toc ~ cyear,bw=23,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes.spdf)
...
model_toc_08<-gwr.basic(cent_toc ~ cyear,bw=382,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes_08.spdf)

pval_toc_08<-gwr.t.adjust(model_toc_08)
```

Using a smaller kernel size knn=90 for comparison.
```{r}
model_toc_90<-gwr.basic(cent_toc ~ cyear,bw=90,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes.spdf)
...
```{r}
ggplot(lakes3, aes(x=stationskoordinat_e_y,y=stationskoordinat_n_x))+
  geom_polygon(data = border1,aes(x = East_SWERE, y = North_SWER),fill = "gray",alpha = 0.25)+
  geom_polygon(data = border_vänern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_vättern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_mälaren,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_point(aes(colour=model_toc_382$SDF$cyear))+
  geom_path(data = border1,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vänern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vättern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_mälaren,aes(x = East_SWERE, y = North_SWER))+
...
```{r}
lakes3%>%mutate(pval0=case_when(pval_toc_382$SDF$cyear_p<0.05 & model_toc_382$SDF$cyear>0~1,
                              pval_toc_382$SDF$cyear_p<0.05 & model_toc_382$SDF$cyear<0 ~-1,
                              TRUE~0))%>%
  group_by(stationskoordinat_e_y, stationskoordinat_n_x)%>%slice(1)->lakes_p_toc_382


ggplot(lakes_p_toc_382, aes(x=stationskoordinat_e_y,y=stationskoordinat_n_x))+
  geom_polygon(data = border1,aes(x = East_SWERE, y = North_SWER),fill = "gray",alpha = 0.25)+
  geom_polygon(data = border_vänern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_vättern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_mälaren,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
...
Slope coefficient for the noncentered data
```{r}
ggplot(lakes3, aes(x=stationskoordinat_e_y,y=stationskoordinat_n_x))+
  geom_polygon(data = border1,aes(x = East_SWERE, y = North_SWER),fill = "gray",alpha = 0.25)+
  geom_polygon(data = border_vänern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_vättern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_mälaren,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_point(aes(colour=model_toc_23_noncent$SDF$cyear))+
  geom_path(data = border1,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vänern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vättern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_mälaren,aes(x = East_SWERE, y = North_SWER))+
...
Significant trends in model with noncentered data for comparison.
```{r}
lakes3%>%mutate(pval0=case_when(pval_toc_23_noncent$SDF$cyear_p<0.05 & model_toc_23_noncent$SDF$cyear>0~1,
                              pval_toc_23_noncent$SDF$cyear_p<0.05 & model_toc_23_noncent$SDF$cyear<0 ~-1,
                              TRUE~0))%>%
...
```{r}
lakes_08%>%mutate(pval0=case_when(pval_toc_08$SDF$cyear_p<0.05 & model_toc_08$SDF$cyear>0~1,
                              pval_toc_08$SDF$cyear_p<0.05 & model_toc_08$SDF$cyear<0 ~-1,
                              TRUE~0))%>%
...
```{r}
lakes3%>%mutate(coef_all=model_toc_382$SDF$cyear,
                pval_all=pval_toc_382$SDF$cyear_p)->lakes_all

lakes_08%>%mutate(coef_08=model_toc_08$SDF$cyear, pval_08=pval_toc_08$SDF$cyear_p)%>%full_join(lakes_all, by=c("ID","year"))%>%
...
Figure S1: Changes in significant trends when stations with high coefficient of variation are removed
```{r}
...
ggplot(diffs1, aes(x=stationskoordinat_e_y.x,y=stationskoordinat_n_x.x))+
  geom_polygon(data = border1,aes(x = East_SWERE, y = North_SWER),fill = "gray",alpha = 0.25)+
  geom_polygon(data = border_vänern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_vättern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_mälaren,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
...
ggplot(lakes3, aes(x=stationskoordinat_e_y,y=stationskoordinat_n_x))+
  ...
  geom_polygon(data = border_vättern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_mälaren,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_point(aes(colour=model_toc_90$SDF$cyear))+
  geom_path(data = border1,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vättern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_mälaren,aes(x = East_SWERE, y = North_SWER))+
  coord_fixed()+
...
Figure 4, right: Significant trends for the model with knn=90

```{r}
lakes_all%>%mutate(pval0=case_when(pval_toc_90$SDF$cyear_p<0.05 & model_toc_90$SDF$cyear>0~1,
                              pval_toc_90$SDF$cyear_p<0.05 & model_toc_90$SDF$cyear<0 ~-1,
                              TRUE~0))%>%
  group_by(stationskoordinat_e_y, stationskoordinat_n_x)%>%slice(1)->lakes_p_toc_90



ggplot(lakes_p_toc_90, aes(x=stationskoordinat_e_y,y=stationskoordinat_n_x))+
  geom_polygon(data = border1,aes(x = East_SWERE, y = North_SWER),fill = "gray",alpha = 0.25)+
  geom_polygon(data = border_vänern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_vättern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_mälaren,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
...
model2008<-gwr.basic(cent_toc ~ cyear,bw=45,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes2008.spdf)
model2009<-gwr.basic(cent_toc ~ cyear,bw=45,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes2009.spdf)
model2010<-gwr.basic(cent_toc ~ cyear,bw=45,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes2010.spdf)
model2011<-gwr.basic(cent_toc ~ cyear,bw=45,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes2011.spdf)
model2012<-gwr.basic(cent_toc ~ cyear,bw=45,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes2012.spdf)
...
#bw2008<-bw.gwr(log_toc ~ cyear,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes2008.spdf)
#bw2012<-bw.gwr(log_toc ~ cyear,adaptive=TRUE,kernel="bisquare", parallel.method=TRUE,data=lakes2012.spdf)
```
...
gwr.point_2008<-ggplot(lakes_2008, aes(x=stationskoordinat_e_y,y=stationskoordinat_n_x))+
  ...
  geom_polygon(data = border_vättern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_mälaren,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_point(aes(colour=model2008$SDF$cyear))+
  geom_path(data = border1,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vänern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vättern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_mälaren,aes(x = East_SWERE, y = North_SWER))+
...
gwr.point_2009<-ggplot(lakes_2009, aes(x=stationskoordinat_e_y,y=stationskoordinat_n_x))+
  ...
  geom_polygon(data = border_vättern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_mälaren,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_point(aes(colour=model2009$SDF$cyear))+
  geom_path(data = border1,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vänern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vättern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_mälaren,aes(x = East_SWERE, y = North_SWER))+
...
gwr.point_2010<-ggplot(lakes_2010, aes(x=stationskoordinat_e_y,y=stationskoordinat_n_x))+
  ...
  geom_polygon(data = border_vättern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_mälaren,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_point(aes(colour=model2010$SDF$cyear))+
  geom_path(data = border1,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vänern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vättern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_mälaren,aes(x = East_SWERE, y = North_SWER))+
...
gwr.point_2011<-ggplot(lakes_2011, aes(x=stationskoordinat_e_y,y=stationskoordinat_n_x))+
  ...
  geom_polygon(data = border_vättern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_mälaren,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_point(aes(colour=model2011$SDF$cyear))+
  geom_path(data = border1,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vänern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vättern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_mälaren,aes(x = East_SWERE, y = North_SWER))+
...
gwr.point_2012<-ggplot(lakes_2012, aes(x=stationskoordinat_e_y,y=stationskoordinat_n_x))+
  ...
  geom_polygon(data = border_vättern,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_polygon(data = border_mälaren,aes(x = East_SWERE, y = North_SWER),fill = "lightblue",alpha = 0.5)+
  geom_point(aes(colour=model2012$SDF$cyear))+
  geom_path(data = border1,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vänern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_vättern,aes(x = East_SWERE, y = North_SWER))+
  geom_path(data = border_mälaren,aes(x = East_SWERE, y = North_SWER))+
...
```{r}
ggarrange(gwr.point_2008,gwr.point_2009, gwr.point_2010, gwr.point_2011, gwr.point_2012, ncol=5, nrow=1, labels = c("2008-2017","2009-2018", "2010-2019", "2011-2020","2012-2021"),hjust=c(-1,-1,-1, -1), common.legend = TRUE, legend="none" )
```

```{r}
lakes_2008%>%mutate(pval0=case_when(pval_2008$SDF$cyear_p<0.05 & model2008$SDF$cyear>0~1,
                              pval_2008$SDF$cyear_p<0.05 & model2008$SDF$cyear<0 ~-1,
                              TRUE~0))%>%
...
lakes_2009%>%mutate(pval0=case_when(pval_2009$SDF$cyear_p<0.05 & model2009$SDF$cyear>0~1,
                              pval_2009$SDF$cyear_p<0.05 & model2009$SDF$cyear<0 ~-1,
                              TRUE~0))%>%
...
lakes_2010%>%mutate(pval0=case_when(pval_2010$SDF$cyear_p<0.05 & model2010$SDF$cyear>0~1,
                              pval_2010$SDF$cyear_p<0.05 & model2010$SDF$cyear<0 ~-1,
                              TRUE~0))%>%
...
lakes_2011%>%mutate(pval0=case_when(pval_2011$SDF$cyear_p<0.05 & model2011$SDF$cyear>0~1,
                              pval_2011$SDF$cyear_p<0.05 & model2011$SDF$cyear<0 ~-1,
                              TRUE~0))%>%
...
lakes_2012%>%mutate(pval0=case_when(pval_2012$SDF$cyear_p<0.05 & model2012$SDF$cyear>0~1,
                              pval_2012$SDF$cyear_p<0.05 & model2012$SDF$cyear<0 ~-1,
                              TRUE~0))%>%
